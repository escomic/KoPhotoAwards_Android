...
package com.devsimtaku.kophoto.core.navigation
...
@Composable
fun NavigationState.toBackStack(): List<NavKey> {
    val backStack by remember(this) {
        derivedStateOf {
            topLevelStack.flatMap { topLevelKey ->
                subStacks[topLevelKey]?.toList().orEmpty()
            }
        }
    }
    return backStack
